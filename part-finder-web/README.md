...
- [admin.html](C:/Users/dell/Documents/Bright%20Sun%20engineering/Sketchup/part-finder-web/admin.html)
  Admin page. Choose the local project folder, then publish those project JSON files to GitHub and update the shared registry.
- [projects.json](C:/Users/dell/Documents/Bright%20Sun%20engineering/Sketchup/part-finder-web/projects.json)
  Shared online project registry. The main user page now checks this file first.
...
If you want the same site to work for everyone, use the hosted registry method.

1. Export the JSON from SketchUp using the LEGNO `Part Finder Web Export` button.
2. Open [admin.html](C:/Users/dell/Documents/Bright%20Sun%20engineering/Sketchup/part-finder-web/admin.html).
3. Click `Choose Project Folder`.
4. Enter your GitHub owner, repository, branch, and a fine-grained token with `Contents: write`.
5. Click `Publish Projects`.
6. The admin page uploads the JSON files into:
   - [data](C:/Users/dell/Documents/Bright%20Sun%20engineering/Sketchup/part-finder-web/data)
   and updates:
   - [projects.json](C:/Users/dell/Documents/Bright%20Sun%20engineering/Sketchup/part-finder-web/projects.json)
7. Wait for GitHub Pages to redeploy.
8. Open the main site:
   - [https://levies4design.github.io/PARTFINDER/](https://levies4design.github.io/PARTFINDER/)
9. Everyone can now choose a project from the dropdown and search by Part Number or Persistent ID.
  ...
  "projects": [
    {
      ...
      "source": "./data/mr-ajith-final.json"
    },
    {
      "key": "foyer-project",
      "name": "Foyer Project",
      "source": "./data/foyer-project.json"
    }
  ]
}
```
...
You can still use the local-folder method for quick testing on one device.
...
The local-folder method stores the project data inside the current browser.

That means:

- it works well for the same device and same browser
- it does not automatically share the data to another person's device
- the shared multi-user method is the hosted `projects.json` registry described above

## Online hosting note

Once the app is published online, the main page first checks the hosted [projects.json](C:/Users/dell/Documents/Bright%20Sun%20engineering/Sketchup/part-finder-web/projects.json).

If that file contains projects, the site works for everyone.

If that file is empty, the app falls back to browser-stored local projects on that device.
