(function () {
  ...
  async function githubGetJsonFile(owner, repo, branch, path, token) {
    const result = await githubApiRequest(`/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/contents/${path}?ref=${encodeURIComponent(branch)}`, token, {
      method: "GET"
    });
    if (result.notFound || !result.data) return null;

    const payload = result.data && result.data.content
      ? decodeGitHubContent(result.data.content)
      : "";
  ...
  async function githubListDirectory(owner, repo, branch, path, token) {
    const result = await githubApiRequest(`/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/contents/${path}?ref=${encodeURIComponent(branch)}`, token, {
      method: "GET"
    });
    if (result.notFound || !Array.isArray(result.data)) return [];
    return result.data;
  }

  async function buildRegistryFromRepoData(owner, repo, branch, token, currentRecords) {
    const currentByKey = new Map(
      (Array.isArray(currentRecords) ? currentRecords : []).map((record) => [record.key, record])
    );

    const entries = await githubListDirectory(owner, repo, branch, "part-finder-web/data", token);
    const directories = entries.filter((entry) => entry && entry.type === "dir" && entry.name);
    const projects = [];

    for (let index = 0; index < directories.length; index += 1) {
      const directory = directories[index];
      const key = String(directory.name);
      let name = currentByKey.has(key)
        ? (currentByKey.get(key).name || key)
        : key;

      const projectPayload = await githubGetJsonFile(
        owner,
        repo,
        branch,
        `part-finder-web/data/${key}/project.json`,
        token
      ).catch(() => null);

      if (!projectPayload && !currentByKey.has(key)) {
        continue;
      }

      if (projectPayload && projectPayload.meta && projectPayload.meta.model_name) {
        name = String(projectPayload.meta.model_name);
      }

      projects.push({
        key,
        name,
        source: `./data/${key}/project.json`
      });
    }

    projects.sort((a, b) =>
      String(a.name || a.key).localeCompare(String(b.name || b.key), undefined, { numeric: true, sensitivity: "base" })
    );
  ...
  async function loadDatasetFromSource(source, options = {}) {
    const raw = String(source || "").trim();
    if (!raw) {
    ...
    try {
      ...
      if (pageMode === "user" && !options.preserveProjectList) {
        setSingleProject(state.datasetName, raw);
      }
  ...
  async function loadProjectRegistry(source) {
    ...
    try {
      ...
      if (project && project.source) {
        await loadDatasetFromSource(project.source, { preserveProjectList: true });
      } else {
  ...
  async function publishProjectsToGitHub() {
    ...
    try {
      ...
      const existingRegistry = await githubGetJsonFile(
        owner,
        repo,
        branch,
        ...
        setGithubStatus(`Registry read warning: ${error.message}. Rebuilding with available projects...`);
        return null;
      });

      const mergedProjectList = await buildRegistryFromRepoData(
        owner,
        repo,
        branch,
        token,
      ...
      const existingDefault = existingRegistry && existingRegistry.default_project
        ? String(existingRegistry.default_project)
        : "";

      const mergedProjectKeys = new Set(mergedProjectList.map((project) => project.key));

      const defaultProject = mergedProjectKeys.has(existingDefault)
        ? existingDefault
        : (records[0] && records[0].key) || (mergedProjectList[0] && mergedProjectList[0].key) || "";

      const registry = {
        default_project: defaultProject,
        projects: mergedProjectList
      };
