(function () {
  ...
  function buildStoredProjectRecord(payload, file, index, usedKeys) {
  ...
  async function loadProjectFolder(files) {
    ...
    try {
      ...
      for (let index = 0; index < jsonFiles.length; index += 1) {
        ...
        records.push(buildStoredProjectRecord(payload, file, index, usedKeys));
      }
      ...
      state.datasetSource = `Local browser storage (${records.length} project${records.length === 1 ? "" : "s"})`;
      loadDataset(firstRecord.payload, firstRecord.name);
  ...
  function refreshSourceSummary() {
    ...
    if (currentProject) {
      els.sourceSummary.textContent = `Project: ${currentProject.name} | Shared source: ${state.datasetSource}`;
      return;
    }
  ...
  async function onClearProjects() {
    try {
      ...
      state.results = [];
      state.activeToken = null;
      els.datasetName.textContent = "No dataset loaded";
      els.datasetMeta.textContent = "Choose a local project folder";
      if (els.generatedLink) els.generatedLink.value = "";
      updateProjectSelect();
      refreshSourceSummary();
      resetSearchUi("Stored local projects cleared. Choose a project folder to continue.");
    } catch (error) {
  ...
  async function publishProjectsToGitHub() {
    ...
    try {
      for (let index = 0; index < records.length; index += 1) {
        ...
        const targetPath = `part-finder-web/data/${record.key}.json`;
  ...
  function loadBrowserStoredProjectsAndSelectFirst(emptyMessage) {
    readAllStoredProjects().then((records) => {
      ...
      loadDataset(firstRecord.payload, firstRecord.name);
      if (els.generatedLink) els.generatedLink.value = buildUserHomeLink();
    }).catch((error) => {
      updateProjectSelect();
      ...
      if (project && project.storage === "browser") {
        try {
          ...
          if (stored && stored.payload) {
            ...
            loadDataset(stored.payload, stored.name || project.name);
          } else {
