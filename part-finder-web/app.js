(function () {
  ...
  async function publishProjectsToGitHub() {
    ...
    try {
      ...
      const existingProjects = existingRegistry && Array.isArray(existingRegistry.projects)
        ? existingRegistry.projects
        : [];

      existingProjects.forEach((project) => {
        if (!project || !project.key) return;
        mergedProjects.set(project.key, {
          key: project.key,
          name: project.name || project.key,
          source: project.source || `./data/${project.key}/project.json`
        });
      });

      records.forEach((record) => {
        mergedProjects.set(record.key, {
          key: record.key,
          name: record.name,
      ...
      const mergedProjectList = Array.from(mergedProjects.values()).sort((a, b) =>
      ...
      const defaultProject = mergedProjects.has(existingDefault)
        ? existingDefault
